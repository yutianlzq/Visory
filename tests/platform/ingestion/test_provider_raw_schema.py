...
def test_raw_schema_registry_covers_both_providers_and_all_datasets() -> None:
    ...
    assert {(item.provider_id, item.dataset_id) for item in records} == {
        ...
        for dataset in ("security_master", "trading_calendar", "bar_1d_raw", "instrument_status_daily", "listing_status_history", "corporate_action", "financial_statement", "benchmark_index_1d")
    }
    assert len({item.expected_schema_hash for item in records}) == 16

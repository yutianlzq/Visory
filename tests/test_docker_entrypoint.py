...
DSA_DOC_ROOT = REPO_ROOT / "docs" / "upstream" / "daily_stock_analysis"


def _read_upstream_doc(relative_path: str) -> str:
    return (DSA_DOC_ROOT / relative_path.removeprefix("docs/")).read_text(
        encoding="utf-8"
    )
...
def test_docker_memory_guides_describe_resource_profiles() -> None:
    ...
    for doc_path in doc_paths:
        doc = _read_upstream_doc(doc_path)

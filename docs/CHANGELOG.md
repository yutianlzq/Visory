...
- [新功能] Visory-G017 / WP-0204 完成独立 `benchmark_index_1d` 指数基准契约、Provider Raw/Canonical Mapping、`0012_wp0204_benchmark_dataset_extension`、`backtest_core` 质量/PIT/非零覆盖/Schema Drift/未处理 Quarantine 门禁与 `CERTIFIED`/`PROVISIONAL`/`PARTIAL`/`UNAVAILABLE`/`STALE` 区分；Formal Backtest 仍只接受 `CERTIFIED`，不连接生产 Provider、不写真实 /data。
...
- [文档] 新增自研 A 股平台架构索引与 Hikyuu 指标/因子/回测设计，固化 T 日预测、T+1 执行、T+H 验证、数据快照、指标物化和全链路追溯边界。
- [文档] 新增评分、仓位与自动权重优化架构，设计全区间研究上界、样本外滚动寻优、AI 有界调仓、复杂均值方差、多机制叠加、权重归因和防前视门禁。
- [文档] 目标产品统一命名为Visory，新服务/Compose/网络使用`visory`前缀，保留`daily_stock_analysis`/DSA作为Legacy底座语义及已确认的`/data/daily_stock_analysis`服务器物理根。
- [文档] 收敛Visory本地优先交付流程和MVP两期：一期在本地完成数据、市场、复盘、L0/L1、策略与Hikyuu回测闭环，二期完成单只L2、安全/恢复生产预演并在Local Release Gate后部署服务器。
- [文档] 固化 A 股回测市场规则 v1：首版覆盖沪深主板、创业板和科创板日线现货多头，统一历史股票池、双轨价格、T+1 保守撮合、涨跌停、整手、容量、日期版本化费用及基准口径。
- [文档] 固化 StrategySpec v1：拆分策略、市场规则、权重、组合和运行契约，定义安全 DSL、筛选/评分/信号/退出分层、受控插件、Hikyuu Compiler 及首批 Sequoia 策略迁移边界。
- [文档] 固化 Fleur-Lite 五阶段回测运行架构：区分 Preview/Research/Formal，采用 PostgreSQL 持久任务和单 Hikyuu Worker，补齐 Run/Attempt、结果原子发布、确定性门禁与轻量资源预算。
- [文档] 固化A股数据平台与Canonical Data Contract v1：以a-stock-data为核心源、Financial-API为补充源，采用带市场前缀的规范股票代码唯一标识，补齐Provider血缘、快照、双源冲突、质量门禁和Hikyuu缓存边界。
- [文档] 固化盘后数据采集与Snapshot发布SLA：交易日16:00启动核心采集，区分Provisional/Certified/Correction，定义Financial-API补充触发、按能力认证、19:00正式策略硬截止和四核资源优先级。
- [文档] 固化A股市场情绪与资金行为架构：定义五维情绪分和状态机，分级管理订单规模、杠杆、公开席位、大宗、板块及互联互通证据，并补齐快照、可信度、T+1回测和盘后调度边界。
- [文档] 固化板块异动、热门观察与策略评分边界：市场域只展示客观指标、独立榜单、公开排名和规则事件，不建立统一热度分；具体策略引用时才生成可追溯、可回测的专属评分。
- [文档] 固化全球市场观察与A股策略隔离：全球指数、汇率、利率、商品和海外事件仅供页面及DSA复盘，不进入Strategy、权重、Paper Portfolio或Hikyuu回测，且失败不阻断A股流水线。
- [文档] 固化A股Feature Store与指标注册中心架构：采用F1/F2/F3三级物化、宽长表混合、依赖DAG和不可变Manifest，明确PIT时间门禁、修订传播、正式引用固定及轻量缓存保留策略。
- [文档] 固化DSA A股收盘复盘FactPack架构：复盘拆分为不可变事实包、带证据引用的AI分析和同源报告，迁移Vibe客观模块，并将T+1观察验证与正式策略回测隔离。
- [文档] 固化A股分层个股研究架构：DSA统一承载L0事实卡、L1快速研究和L2深度研究，迁移UZI覆盖与自查方法及TradingAgents七角色辩论，并通过StockResearchFactPack隔离研究观点与策略。
- [文档] 固化A股主平台底座：以DSA React/FastAPI构建模块化单体，定义统一页面与API分域、PostgreSQL持久任务、单重Worker优先级、owner/viewer权限及兼容迁移路线。
- [文档] 固化A股平台公网部署安全：采用Cloudflare橙云、Full Strict、NPM反代与DNS-01证书，公网v1仅允许唯一owner密码并在登录时强制Turnstile服务端验证，统一`/data/daily_stock_analysis`挂载、源站隔离、备份和恢复。
- [文档] 建立A股平台契约收敛总纲并确认C-001：采用`entity_key=asset_type:canonical_id`隔离资产身份，统一PIT时间字段，拆分发布、修订、质量、任务、执行阶段和研究门禁状态，规范版本及确定性Hash。
- [文档] 收敛A股平台C-002至C-013实现契约：固定UUIDv7资源ID、Alias/Resolver、`storage/app`唯一数据根、原子发布，以及Provider、Snapshot、Feature、Task、Hikyuu、FactPack、API、调度、部署和MVP验收对象。
- [文档] 新增Claude Code自研A股平台开发总指引、需求功能闭环、12页低保真原型、工程编码规范和M0—M9实施路线，固化2018年至今核心回填范围及FR—页面—API—任务—产物—测试追踪。
...
- openclaw Skill 集成指南 — 新增 [docs/openclaw-skill-integration.md](https://github.com/ZhuLinsen/daily_stock_analysis/blob/96bc532dfcb21e3a7d0fdbdfa87d764b9b61d2ee/docs/openclaw-skill-integration.md)，说明如何通过 openclaw Skill 调用 DSA API

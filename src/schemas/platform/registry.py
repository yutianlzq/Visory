...
PLATFORM_CONTRACTS = ContractRegistry(
    (
        ...
        ContractRegistration(
            contract_id="C-004/DataSnapshot",
            owner_module="src.schemas.platform.snapshot",
            producer="Snapshot Gate Service",
            consumers=("feature store", "review", "research", "backtest"),
            schema_model=DataSnapshot,
            schema_version="1.0.0",
            business_key="trade_date + cutoff_at + revision",
            resource_id_field="snapshot_id",
            time_semantics=("trade_date", "cutoff_at", "available_at", "created_at", "published_at"),
            version_semantics=("manifest_version", "provider_policy_version", "revision", "revision_kind"),
            quality_semantics=("quality_status", "publication_status", "certified_capabilities", "missing_capabilities"),
            lineage_fields=("provider_policy_id", "security_master_ref", "calendar_ref", "canonical_partitions", "supersedes_id"),
            storage_profile="PostgreSQL data_snapshot plus deterministic manifest StorageRef binding",
            retention_class="PINNED",
            compatibility="snapshots are immutable and corrections append a new revision without replacing the superseded snapshot",
            golden_payloads=_golden("success/data-snapshot.json", "correction/data-snapshot-correction.json", "rejected/data-snapshot-rejected-published.json"),
        ),
        ContractRegistration(
            contract_id="C-004/SnapshotPartitionRef",
            owner_module="src.schemas.platform.snapshot",
            producer="Snapshot Gate Service",
            consumers=("snapshot", "capability engine"),
            schema_model=SnapshotPartitionRef,
            schema_version="1.0.0",
            business_key="canonical_partition_id + snapshot_id",
            resource_id_field="canonical_partition_id",
            time_semantics=("min_available_at", "max_available_at"),
            version_semantics=("dataset_schema_version", "provider_policy_version", "revision", "revision_kind"),
            quality_semantics=("quality_status", "partition_hash", "schema_hash", "row_count"),
            lineage_fields=("provider_run_refs", "raw_object_refs", "quality_report_id", "storage_ref"),
            storage_profile="embedded immutable canonical partition reference",
            retention_class="PINNED",
            compatibility="references retain canonical lineage and content identity without copying runtime paths",
            golden_payloads=_golden("success/snapshot-partition-ref.json"),
        ),
        ContractRegistration(
            contract_id="C-004/CapabilityCertification",
            owner_module="src.schemas.platform.snapshot",
            producer="Snapshot Capability Engine",
            consumers=("snapshot gate", "formal consumers", "operations"),
            schema_model=CapabilityCertification,
            schema_version="1.0.0",
            business_key="snapshot_id + capability_id",
            resource_id_field="snapshot_id",
            time_semantics=("certified_at",),
            version_semantics=(),
            quality_semantics=("capability_status", "reason_code", "evidence_refs"),
            lineage_fields=("snapshot_id", "evidence_refs"),
            storage_profile="PostgreSQL capability_certification",
            retention_class="AUDIT",
            compatibility="capability certification is independent, explicit, and never inferred from consumer success",
            golden_payloads=_golden("success/capability-certification.json", "rejected/capability-certified-with-reason.json"),
        ),
        ContractRegistration(
            contract_id="C-004/ConsumerRequirement",
            owner_module="src.schemas.platform.snapshot",
            producer="Snapshot Consumer",
            consumers=("preview", "formal backtest", "research"),
            schema_model=ConsumerRequirement,
            schema_version="1.0.0",
            business_key="consumer_id",
            resource_id_field=None,
            time_semantics=(),
            version_semantics=("requirement_version",),
            quality_semantics=("required_capabilities", "accepted_publication_statuses", "min_quality_status", "allow_provisional"),
            lineage_fields=(),
            storage_profile="PostgreSQL consumer_requirement",
            retention_class="PINNED",
            compatibility="formal backtest consumers require CERTIFIED snapshots and backtest_core",
            golden_payloads=_golden("success/consumer-requirement-preview.json", "success/consumer-requirement-formal.json", "rejected/consumer-requirement-formal-provisional.json"),
        ),
        ContractRegistration(
            contract_id="C-004/SnapshotCurrentPointer",
            owner_module="src.schemas.platform.snapshot",
            producer="Snapshot Gate Service",
            consumers=("current snapshot readers", "feature store", "backtest"),
            schema_model=SnapshotCurrentPointer,
            schema_version="1.0.0",
            business_key="scope + trade_date + capability_id",
            resource_id_field="snapshot_id",
            time_semantics=("trade_date", "updated_at"),
            version_semantics=("pointer_revision", "previous_snapshot_id"),
            quality_semantics=("compare-and-set update", "certified capability target"),
            lineage_fields=("snapshot_id", "previous_snapshot_id"),
            storage_profile="PostgreSQL snapshot_current_pointer with row locking",
            retention_class="PINNED",
            compatibility="pointer updates are CAS-protected and preserve prior certified target for rollback",
            golden_payloads=_golden("success/snapshot-current-pointer.json"),
        ),
        ContractRegistration(
            contract_id="C-004/SnapshotBuildTaskRequirements",
            owner_module="src.schemas.platform.snapshot",
            producer="Platform API client",
            consumers=("Snapshot Build Worker",),
            schema_model=SnapshotBuildTaskRequirements,
            schema_version="1.0.0",
            business_key="trade_date + cutoff_at + provider_policy + canonical_partition_ids",
            resource_id_field=None,
            time_semantics=("trade_date", "cutoff_at"),
            version_semantics=("provider_policy_version",),
            quality_semantics=("requested_capabilities", "publication_status"),
            lineage_fields=("security_master_ref", "calendar_ref", "canonical_partition_ids", "correction_of_snapshot_id"),
            storage_profile="durable Task requirements without runtime filesystem paths",
            retention_class="AUDIT",
            compatibility="only supported for task_type data_snapshot_build and keeps correction lineage explicit",
            golden_payloads=_golden("success/snapshot-build-task-requirements.json"),
        ),
        ContractRegistration(
            contract_id="C-004/SnapshotBuildTaskResult",
            owner_module="src.schemas.platform.snapshot",
            producer="Snapshot Build Worker",
            consumers=("task control", "operations", "snapshot consumers"),
            schema_model=SnapshotBuildTaskResult,
            schema_version="1.0.0",
            business_key="task_id + attempt_id",
            resource_id_field=None,
            time_semantics=(),
            version_semantics=(),
            quality_semantics=("published", "requirement_status", "failure_code", "capability_certifications"),
            lineage_fields=("task_id", "attempt_id", "snapshot"),
            storage_profile="worker result without arbitrary filesystem paths",
            retention_class="AUDIT",
            compatibility="failed builds never expose a snapshot and successful results include capability evidence",
        ...
        ContractRegistration(
            contract_id="C-004/CanonicalQualityReport", owner_module="src.schemas.platform.canonical", producer="Canonical Normalizer", consumers=("canonical partition", "operations"), schema_model=CanonicalQualityReport, schema_version="1.0.0", business_key="quality_report_id", resource_id_field="quality_report_id", time_semantics=("created_at",), version_semantics=("dataset_schema_version", "mapping_version"), quality_semantics=("failed quality blocks publication", "mapping_hash and input refs make failures traceable"), lineage_fields=("canonical_partition_id", "task_id", "attempt_id", "dataset_id", "provider_run_refs", "raw_object_refs"), storage_profile="PostgreSQL canonical_quality_report", retention_class="AUDIT", compatibility="quality failures remain auditable and traceable to task, mapping, ProviderRun and RawObject", golden_payloads=_golden("success/canonical-quality-report.json"),
        ),
        ContractRegistration(
